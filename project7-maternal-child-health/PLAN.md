...
- [x] Age-band reconciliation — team decided on **12–35 months**, standardized across stunting, immunisation, and SBA (2026-08-04, see `Presentation.Rmd` § Approach)
- [x] National stunting/immunisation/SBA prevalence recomputed on the 12–35mo band (21.7% / 50.3% / 88.8%) — see § App update (2026-08-04)
- [x] Immunisation and SBA analysis — national prevalence, county forest plots/maps, and wealth-category breakdown all surfaced in-app
- [x] Small-area estimation (MBG + INLA) — observed prevalence, diagnostics, and predicted-probability maps for all three indicators, surfaced on Spatial & Bayesian Analysis
- [x] Task 4 v2: multi-indicator vulnerability ranking (stunting + immunisation + SBA), surfaced on Intervention Prioritization
- [x] SDG links for all three indicators — Problem Statement page
- [ ] Discrete 5-quintile wealth breakdown — a 3-category (Low/Middle/High) breakdown is available instead (`figures/fig-wealth-category-indicators-1.png`)
- [ ] Regenerate the v1 stunting ranking on the 12–35mo band (still runs on the earlier 6–59mo county CSV — see § App update, 2026-08-04)
...
- Not attempted (as of 2026-08-03): an interactive county choropleth (no
  vetted GeoJSON boundary file exists in this repo) and any INLA/MBG
  numeric output. **Superseded 2026-08-04** — see below; the team's own
  environment ran both models and the rendered outputs are now in-app.

## App update (2026-08-04)

`origin/task2-regional-indicators` gained 14 more commits, this time
including **rendered outputs**, not just source: `Presentation.Rmd` (the
team's slide deck, with SDG mappings and the MBG model equations),
`MCH_Kenya_Task2_Task4_Pipeline.qmd` (a 2,000+ line consolidated,
16-phase Task 2 + Task 4 pipeline superseding the earlier separate
scripts), and `Plots/` / `figures/` — real PNGs the team generated by
actually running the analysis, including MBG/INLA diagnostics and
predictions that were previously only unrun source.

**Resolved by this pull:**
- The **age-band decision** flagged since 2026-08-02 is made: all three
  indicators are standardized to **12–35 months** (`Presentation.Rmd` §
  Approach: "Youngest living child aged 12-35 months, unweighted n = 6022").
- **National prevalence for all three indicators**, read from the annotated
  reference line in each indicator's forest plot (the same recovery method
  used for the concentration indices on 2026-08-02 — the plot annotations
  evaluate correctly even where the `.qmd`'s separate inline-prose stats
  don't): stunting 21.7%, full immunisation 50.3%, SBA 88.8%.
- **MBG and INLA are no longer just prep** — `Plots/combined_diagnostics_graphs.png`,
  `combined_predicted_graphs.png`, `combined_variograms_plots.png`,
  `combined_correlation_plots.png`, and `combined_cluster_observed_prev_graphs.png`
  are real, rendered model outputs, now on the Spatial & Bayesian Analysis page.
- **Task 4 v2** (multi-indicator vulnerability ranking) exists as a real
  rendered output (`figures/fig-top-counties-ranking-1.png`), now on the
  Intervention Prioritization page.
- **Wealth-category (Low/Middle/High) breakdown** for all three indicators
  (`figures/fig-wealth-category-indicators-1.png`) — not a 5-quintile
  table, but a real, complementary discrete-wealth view.

**Still open:**
- The **v1 stunting county ranking** (`task4_priority_counties.csv`,
  `task2_stunting_by_county.csv`) still runs on the earlier 6–59 month
  band — no exact county-level 12–35mo CSV exists in this repo (only the
  forest-plot/map images), so the app keeps v1's threshold math tied to
  its own band (`NATIONAL_STUNTING_PCT_6_59_LEGACY` in `app/utils.py`)
  rather than silently mixing bands. The v2 ranking above already reflects
  the current 12–35mo standard.
- A true discrete 5-quintile wealth CSV (as opposed to the 3-category
