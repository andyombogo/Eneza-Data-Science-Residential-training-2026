---
title: "How Kenyan maternal/child indicators (stunting, skilled birth attendance, immunisation) vary by region and wealth quintile - Descriptive and exploratory analysis"
...
#| fig-width: 5
#| fig-height: 6
#| fig-cap: "Weighted prevalence of stunting among children 12-35 months at the subnationa levels"
        
## Prevalence maps
map_stunting <- plot_map(
    merged_stunting, fill = prevalence,
    breaks = c(0, 10, 20, 30, 40, 50),
    labels = c("0–9.9", "10–19.9", "20–29.9", "30–39.9", "40–49.9"),
...
@fig-prev-map shows the weighted prevalence of stunting among children 12-35 months at the subnational levels. Darker shades shows increasing prevalence
...
## Pulling adjusted OR from the DHS survey weighted analysis
or_estimates <- read_csv(
  "./Data/Derived_estimates/dhs_stunting_WI_adjusted_OR.csv") |> 
    mutate(
      estimate = ifelse(
        label == "Low" & is.na(estimate), "Ref", estimate),
      p.value = ifelse(
        label == "Low" & is.na(p.value), "Ref", p.value)) |> 
    filter(
...
cc_stunting <- cc_plot_data |> 
  ...
  annotate(
    ...
    colour = "black") +
  labs(
    x = "Cumulative % of children, ranked by wealth (Low \u2192 High)",
...
cc_vaccination <- cc_plot_data_immunn |> 
...
Weighted prevalence of  sba - subnational levels
...
cc_sba <- cc_plot_data_sba |> 
  ...
  annotate(
    ...
    colour = "black") +
  labs(
    x = "Cumulative % ranked by wealth (Low \u2192 High)",
...
ggsave(
  "./Plots/combined_maps.png",
  (map_stunting | map_vaccination | map_sba),
  width = 12,
  height = 8,
  dpi = 500,
  bg = "white")
...
ggsave(
  "./Plots/combined_cc_graphs.png",
  (cc_stunting | cc_vaccination) / (cc_sba | plot_spacer()),
  width = 14,
  height = 10,
  dpi = 500,
  bg = "white")
